...
def movement(controller, move):
    ...
    if len(move[3]):
        # pass
        xtranslate = float(move[3].sum()) * gain  #forward distance
        xtranslate = -xtranslate*2.54/100
    # x axis front mouse / side mouse
    if len(move[0]) and len(move[2]): #float(move[2].sum()) + 
        zr = (float(move[0].sum()))* gain
        zrotate = zr/7
        
    print("rotate", "%.3f" % zrotate, "translate", "%.3f" % xtranslate)
    # Get the actuators
    ...
    print("current position", "%.3f" % pos[0], "%.3f" % pos[1], "%.3f" % ori[1][0], "%.3f" % ori[0][0])

    pos_n = [xtranslate*ori[0][0]+pos[0], xtranslate*ori[1][0]+pos[1]]
    ...
    act_xtranslate.dLoc = [xtranslate, 0, 0]
